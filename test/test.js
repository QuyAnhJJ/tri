...
describe('Splitwise Contract', function () {
  ...
  it('should add a debt successfully', async function () {
    const amount = 100;
    await splitwise.add_IOU(addr1.address, amount, [], 0);
  ...
  it('should not allow adding debt to oneself', async function () {
    await expect(splitwise.add_IOU(owner.address, 100, [], 0))
      .to.be.revertedWith('Debtor equal to creditor!');
  });

  it('should lookup the correct debt amount', async function () {
    const amount = 200;
    await splitwise.add_IOU(addr1.address, amount, [], 0);
    const debt = await splitwise.lookup(owner.address, addr1.address);
    expect(debt).to.equal(amount);
  ...
  it('should correctly reflect debts after adding IOUs', async function () {
    ...
    await splitwise.add_IOU(addr1.address, 100, [], 0); 
    await splitwise.connect(addr1).add_IOU(addr2.address, 200, [], 0); 
